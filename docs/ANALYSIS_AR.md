...
## 5b) جولة التطوير الثانية (v1.3.0)

نُفّذت بعد التقرير الأول بناءً على طلب المستخدم (ميزات جديدة + إصلاحات + تحسين UI + بناء APK):

- **UI**: وحدة `ProcessingUiLabels` تعرّب كل مراحل المعالجة عبر الشاشات (Home، Projects، Usage
  Dashboard، Upload)؛ بطاقة «المعالجة النشطة» تظهر أثناء عمل مهمة الخلفية مع زر إلغاء؛ إزالة خيار
  «4K» الوهمي من نافذة التصدير مع توضيح أن الترميز بدقة المصدر.
- **ميزات**: إعادة محاولة المهام الفاشلة/الملغاة بلمسة واحدة من المشاريع (`retryVideoProcessing`
  يعيد جدولة نفس jobId)؛ نسخ SRT/VTT من نافذة التصدير.
- **إصلاحات**: بطاقة استئناف المسودة لا تلصق روابط محلية منتهية في حقل URL؛ العامل يُبقي نسخة
  المصدر عند الفشل النهائي ليعمل زر إعادة المحاولة؛ إصلاح استيراد مفقود رصده CI.
- **الإصدار**: bump إلى `1.3.0` (versionCode 6) وبناء APK ونشره Release:
  https://github.com/ISM-dragon/-/releases/tag/android-v1.3.0
